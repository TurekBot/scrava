...
Viewing this page in a browser will display the form. You can submit a submission by
...
requests for `/submission` by returning the name of a {View}[`View`] (in this case,
`submission`). The following `View` is responsible for rendering the HTML content:
...
methods in this controller are both mapped to `/submission`. You can use `@RequestMapping`
...
This mapping lets the controller differentiate the requests to the `/submission` endpoint.
...
`id` and `content` that correspond to the form fields in the `submission` view and are used
...
rendering of the HTML by converting the view name (`submission`) into a template to render.
In this case, we use https://www.thymeleaf.org/doc/html/Thymeleaf-Spring3.html[Thymeleaf],
which parses the `submission.html` template and evaluates the various template expressions
to render the form. The following listing (from
`src/main/resources/templates/submission.html`) shows the `submission` template:

====
[source,html]
----
include::complete/src/main/resources/templates/submission.html[]
----
====

The `th:action="@{/submission}"` expression directs the form to POST to the `/submission`
endpoint, while the `th:object="${submission}"` expression declares the model object to use
...
process of submitting the form. As noted earlier, the form submits to the `/submission`
...
`submission` in this case). The `id` is rendered in the
`<p th:text="'id: ' + ${submission.id}" />` expression. Likewise, the `content` is rendered
in the `<p th:text="'content: ' + ${submission.content}" />` expression. The following
...
Now that the web site is running, visit http://localhost:8080/submission, where you see the
following form:
